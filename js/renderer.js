...
function drawOverlay() {
  ...
  if (isTilted) drawBuildingExtrusions(ctx);
...
// flat/top-down mode buildings keep rendering as plain footprints on the main canvas
// (see the "building" branch above and the state.buildingFeatures loop below), untouched
// by this function. Painter's-algorithm sorted (farthest building first) so nearer
// buildings correctly draw over farther ones.
function drawBuildingExtrusions(ctx) {
  if (!state.buildingFeatures || !state.buildingFeatures.length) return;
  if (!tiltActive() || !state.userLocation) return;
  ...
  for (const feature of state.buildingFeatures) {
    const heightMetres = feature.properties && feature.properties.heightMetres;
    if (!heightMetres) continue;

    const geometry = feature.geometry;
    if (!geometry) continue;
    // MultiPolygon buildings and courtyard holes are rare in this extract (all current
    // features are simple Polygons) -- only the outer ring of the first polygon is
    // extruded, matching the simplification estimateBuildingHeightMetres already makes
    // for footprint area.
    const polygons = polygonRingsFromGeometry(geometry);
    const outerRing = polygons[0] && polygons[0][0];
    if (!outerRing || outerRing.length < 3) continue;

    // Cheap distance cull on the first vertex before doing any real projection work --
    // exact enough given buildings are small relative to the cull radius.
    const firstWorld = projectLonLat(Number(outerRing[0][0]), Number(outerRing[0][1]));
    const dx = firstWorld.x - userPoint.x;
    const dy = firstWorld.y - userPoint.y;
    const distSq = dx * dx + dy * dy;
    if (distSq > maxWorldDistanceSq) continue;

    // Negative/zero = ahead (tiltRotatedHeadingOffset, also used by isBehindTiltHeading
...
function drawSingleBuildingExtrusion(ctx, ring, heightMetres, alpha) {
  const groundPoints = [];
  const roofPoints = [];
  for (let i = 0; i < ring.length; i += 1) {
    const worldPoint = projectLonLat(Number(ring[i][0]), Number(ring[i][1]));
    groundPoints.push(worldToScreenForOverlayTilted(worldPoint));
    roofPoints.push(worldToScreenForOverlayTiltedAtHeight(worldPoint, heightMetres));
  }
