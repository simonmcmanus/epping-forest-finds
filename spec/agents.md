...
- **A data change that puts a new pin on screen fails `02-overview.spec.js`'s snapshot, and that is the change working.** The weekly ledger adds and removes places near Loughton High Road, which is inside the Nearby screen's frame, so most weekly data pull requests make the committed image out of date. It is not a regression and the baseline is not wrong — it is describing a map that no longer exists. Run the **Update Snapshots** workflow against that branch before merging; CI stays red until you do. Never regenerate it anywhere but a Linux runner (see `PLAYWRIGHT_CHROMIUM_EXECUTABLE` below).
