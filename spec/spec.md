...
- **Relocating stays in the nearby view.** Moving the browse anchor slides the same view onto the new spot — same zoom, ring the same size on screen, centred the same way — so repeated browsing steps from place to place rather than progressively zooming out. Every further tap outside the (now moved) ring relocates again.
