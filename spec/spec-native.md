...
- **Android back button** — the web router already gives the system back button the right behaviour: every screen change pushes a history entry, so back steps out of a selection, then out of Filters, and only exits once the trail is spent (see [spec.md](spec.md) § URL hash / navigation state). The Capacitor shell only needs to let the WebView handle it rather than backgrounding the app itself.
