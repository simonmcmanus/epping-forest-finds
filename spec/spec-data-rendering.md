...
- When `nearbyHeadingUpActive()` is true, tree/landmark/cow pins are also moved from `#mapCanvas` to `#overlayCanvas` (drawn in `drawOverlay()` before the user dot, using `worldToScreenForOverlay()`). This prevents all location pins from rotating with the CSS-rotated map canvas — they stay at their correct geographic positions on screen exactly as they do in selected navigation mode.
...
- **Camera fit target (`overviewTargetPoints`):** when two or more in-radius items exist, returns only the item points (no user location) so the item cluster fills the screen regardless of how far the user is from the cluster. When fewer than two items exist, user location is prepended so there is always a meaningful centring anchor. The viewport fits the geographic bounding box of these points — if items are clustered close the map zooms in tight; if they spread to the radius edge the map shows that full extent. The walking-radius distance is never used directly to set the zoom.
