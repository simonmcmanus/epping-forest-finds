...
- Entering heading-up mode: rotation from 0° to compass heading is animated over the same duration as the viewport pan/zoom (600 ms default), driven by `state.rotationAnimStartTime` in `prepareCanvasForDraw`; `renderer.js` keeps scheduling redraws until the animation completes.
- `animateToHeadingUpNavigationViewport(durationMs)` is the single authoritative function for transitioning to heading-up view — it starts both the viewport animation and the rotation animation together.
