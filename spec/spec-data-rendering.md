...
- **Camera-facing GPS smoothing (`ingestLocationFix` / `advanceLocationGlide`).** `watchPosition` delivers roughly one fix a second and consecutive fixes wander several metres under tree cover, while `state.userLocation.point` is the camera anchor, the scale fit's origin and the 3D pivot at once (`cameraOriginPoint`) — so a raw fix moved the whole map at once. Measured walking toward a destination ~80 m away with the heading held still, the map lurched 15.8 px on average and 26.2 px at worst on each fix frame against 0.3 px on the frames between: a once-a-second twitch. Two separate problems get two separate mechanisms. The *wander* is noise around the truth, so `ingestLocationFix` low-passes each fix into a target with an exponential filter whose time constant is the reported `accuracy` over `LOCATION_SMOOTHING_ACCURACY_DIVISOR` (10), clamped to `LOCATION_SMOOTHING_MIN_TAU_S`–`LOCATION_SMOOTHING_MAX_TAU_S` (0.3–2.5 s), so a 5 m fix is followed almost as given and a 25 m one is leaned on hard; it filters against the previous *target* rather than the current position, since filtering against a value that is itself mid-glide would fold the glide's lag back in and drag the position permanently behind the walker. The *once-a-second delivery* would make even a perfectly filtered position arrive as a step, so the position then glides to that target over `LOCATION_GLIDE_MS` (850 ms, a little under the fix interval), advanced every frame by `advanceLocationGlide()` from `prepareCanvasForDraw`. Glide progress is a pure function of the clock, the same shape `nearbyOriginTransitionEasedProgress` uses, so a dropped frame just means further along and a stale clock lands it — neither can freeze the position or leave it somewhere the fit was not solved for. A move of `LOCATION_SMOOTHING_SNAP_METRES` (30 m) or more is not wander — a first fix after a gap, or coming out of a tunnel — and lands at once. Because the glide writes `state.userLocation` itself, the camera, the fit, the route head and the "You" marker all move together and cannot disagree. Result on the same walk: 0.5 px average and 3.5 px worst on a fix frame, against 0.4 px between — motion spread evenly across frames instead of concentrated in one. Both live in the two GPS ingestion paths (`applyLocationFix` and the `watchPosition` callback) rather than in a getter, so anything that assigns `state.userLocation` directly is unaffected. `state.rawUserLocation` keeps the unfiltered fix.
