...
- The directional arrow uses the same 20px footprint as the nearby item icon so the list stays visually aligned.
...
- **Heading-up nearby mode:** when in overview/nearby mode (no selection, no filter screen) with user location and compass heading available, `nearbyHeadingUpActive()` returns true and the map also switches to heading-up. The user is anchored at the vertical center (50%) rather than 62% since there is no navigation destination to keep visible above the user. On first activation an entry animation rotates from north-up to the current heading over 500 ms, driven by the same `headingUpEntryAnim` mechanism. Drag, zoom and resize re-center on the user via `alignHeadingUpNavigationViewport()`. The `headingUpActive()` helper returns true for either selected-navigation or nearby heading-up and is the single check used in the compass smoothing tick and canvas transform logic. The user radar cone points toward the top of the screen in both heading-up modes. Tree, landmark, and cow pins are drawn on `#overlayCanvas` (not `#mapCanvas`) so they remain at their correct geographic positions on screen as the map CSS-rotates — the same mechanism used for the selected-item pin in navigation mode.
