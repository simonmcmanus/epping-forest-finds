...
- **Search's camera and highlighting are its own, not the shared "survey mode" fit above.** Every recompute of the results (`searchResultsHtml`, run on open and on each debounced keystroke) calls `updateSearchHighlight()`, which keeps `state.searchHighlightResults` to the top `SEARCH_HIGHLIGHT_LIMIT` (10) matches, nearest first (`searchMapFeatures()`'s own rank-then-distance sort — a broad query like a place category ranks its matches equally, so distance from `nearbyOrigin()` decides the order and a "cows" search leads with the nearest cow), and re-draws the highlighted pins immediately. The camera itself does not: `fitSearchCameraToHighlight()` (which fits `fitToPoints`, animated, `focusVisibleArea`, to all the highlighted points plus `nearbyOrigin()`'s own point at once — so a broad query zooms out just enough to fit every top match *and* keep the user's own position in view, the same origin their walk-chip distances are measured from) only runs after `SEARCH_CAMERA_FIT_DEBOUNCE_MS` (350ms) of no further query change. Re-fitting on every keystroke — what this did originally — meant a fast typist's transient, in-progress result sets (a single very-close match a few characters into a longer name is common) each got their own camera move, reading as a jumpy zoom-in-then-back-out before the query settled; the highlighted pins still update every keystroke; only the camera waits for a pause. `updateSearchHighlight` cancels any pending fit before scheduling the next one, and the deferred fit bails if Search has since closed (`state.searchScreenOpen`) — picking a result or leaving Search can't be overtaken by a stale fit landing moments later. With no location fix, there is no origin point to add and the fit covers the matches alone. An empty or too-short query clears the highlight, cancels any pending fit, and leaves the camera alone; opening Search with a query already in `state.searchQuery` (returning from a result) frames that query's matches instead of falling back to the walking-radius "survey mode" fit the other three screens use.
