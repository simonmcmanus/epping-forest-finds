...
- **Camera fit target (`overviewTargetPoints`):** two zoom modes depending on UI state:
  ...
  - **Survey mode (filter panel open):** anchors to the user location plus the nearest `nearestItemsCount` items per active type (ignoring the walking radius), so the user can see how far away transport or other points of interest are. The user location is always included in the fit. Opening the filter panel triggers an animated zoom to this wider view; closing it (via the Nearby button or back navigation) snaps back to foraging zoom. Filter toggles within the filter panel update the survey zoom immediately.
