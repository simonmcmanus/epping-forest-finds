...
- **Search's camera and highlighting are its own, not the shared "survey mode" fit above.** Every recompute of the results (`searchResultsHtml`, run on open and on each debounced keystroke) calls `updateSearchHighlight()`, which keeps `state.searchHighlightResults` to the top `SEARCH_HIGHLIGHT_LIMIT` (10) matches and fits the camera (`fitToPoints`, animated, `focusVisibleArea`) to all of their points at once — so typing a broad query zooms out just enough to fit every top match, and narrowing the query re-fits tighter as results drop out. `drawSearchHighlights()` (`js/renderer.js`) rings each of those same points on the overlay canvas while `state.searchScreenOpen` is true, so the highlighted set and the framed set are always the same items. An empty or too-short query clears the highlight and leaves the camera alone; opening Search with a query already in `state.searchQuery` (returning from a result) frames that query's matches instead of falling back to the walking-radius "survey mode" fit the other three screens use.
