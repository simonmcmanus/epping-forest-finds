...
- **Heading-up nearby mode:** when there is no real navigation target (`selectedCompassTarget()` returns null) but user location and compass heading are available, `nearbyHeadingUpActive()` returns true and the map switches to heading-up. This covers the overview/nearby screen, the filter panel, and pseudo-selection screens (settings, report) — all of which show the map in the background and should continue rotating with the device heading. The user is anchored lower in the visible map (around 72%) and the nearby refit keeps every highlighted item inside the visible area; if a highlighted item sits behind the user, the camera still repositions so that item stays close to the bottom edge instead of falling back to a centered view. On first activation an entry animation rotates from north-up to the current heading over 500 ms, driven by the same `headingUpEntryAnim` mechanism. Drag, zoom, resize, compass changes, and nearby filter changes all re-fit the overview targets via `alignHeadingUpNavigationViewport()` / `ensureOverviewTargetsVisible()`. The `headingUpActive()` helper returns true for either selected-navigation or nearby heading-up and is the single check used in the compass smoothing tick and canvas transform logic. The user radar cone points toward the top of the screen in both heading-up modes. Tree, landmark, and cow pins are drawn on `#overlayCanvas` (not `#mapCanvas`) so they remain at their correct geographic positions on screen as the map CSS-rotates — the same mechanism used for the selected-item pin in navigation mode.
...
- **Overview mode GPS follow — filters active:** `ensureOverviewTargetsVisible` keeps all active highlighted items inside the visible map area and animates the refit when the overview target set changes. In nearby heading-up mode this uses the heading-aware camera fit so the user can sit low on the map when the highlighted items are all ahead. This ensures fast-moving users (walking, train) never lose sight of nearest items. Animation only runs on significant movement.
