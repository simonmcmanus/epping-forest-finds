...
Search, Filter, Settings, and Report share identical navigation behaviour, gated by the shared `secondaryScreenActive()` helper (`state.searchScreenOpen || state.filterScreenOpen || state.selected?.type === "settings" || state.selected?.type === "report"`). Where the points below say "all three", Search is the fourth and behaves the same — except for its camera fit and its highlighted set, which are its own (see "Search's camera and highlighting" below):
...
- **Search results are drawn from the whole dataset, not the ring.** The other three screens are views of what is around you; Search is not, so its list is built by `searchMapFeatures()` over a name index of every named feature (`ensureSearchIndex`), independently of the walking radius and of the active filters. See "Search Screen" in `spec.md`.
- **Search's camera and highlighting are its own, not the shared "survey mode" fit above.** Every recompute of the results (`searchResultsHtml`, run on open and on each debounced keystroke) calls `updateSearchHighlight()`, which keeps `state.searchHighlightResults` to the top `SEARCH_HIGHLIGHT_LIMIT` (10) matches, nearest first (`searchMapFeatures()`'s own rank-then-distance sort — a broad query like a place category or a bare species word ranks its matches equally, so distance from `nearbyOrigin()` decides the order and a "cows" or "oak" search leads with the nearest one), and re-draws the highlighted pins immediately.
  - **The camera fits immediately the moment a query goes from no matches to some** (`fitSearchCameraToHighlight()`, fitting `fitToPoints`, animated, `focusVisibleArea`, to all the highlighted points plus `nearbyOrigin()`'s own point at once — so a broad query zooms out just enough to fit every top match *and* keep the user's own position in view, the same origin their walk-chip distances are measured from). Debouncing that first fit (an earlier version of this did) left the camera sitting on whatever it was framing before — the Nearby ring, most often — for the length of the debounce, which read as the map still showing Nearby even though a match already existed.
  - **Every fit after that first one, while results are still showing, waits for `SEARCH_CAMERA_FIT_DEBOUNCE_MS` (350ms) of no further query change.** Re-fitting on every keystroke here (rather than just the first) meant a fast typist's transient, in-progress result sets — a single very-close match a few characters into a longer name is common — each got their own camera move, reading as a jumpy zoom-in-then-back-out before the query settled. The highlighted pins still update every keystroke regardless; only these later re-fits wait for a pause. `updateSearchHighlight` cancels any pending fit before scheduling the next one, and the deferred fit bails if Search has since closed (`state.searchScreenOpen`) — picking a result or leaving Search can't be overtaken by a stale fit landing moments later.
  - **Losing every match falls back to the Nearby ring immediately** (`ensureOverviewTargetsVisible()`, the same "survey mode" fit the other three screens use) rather than leaving the camera stranded on the last search fit — but only when there *were* matches a moment ago; an empty query on first opening Search has nothing to fall back from (`openSearchScreen`'s own fallback already covers that case). With no location fix, there is no ring to fall back to and no origin point to add to a fit — the camera simply does not move on an empty query.
  - **A GPS fix or compass tick can't reassert the ring over a search that still has matches**, either: `alignHeadingUpNavigationViewport()` — the ring's own framing, reached both directly from every `watchPosition` fix and indirectly through `ensureOverviewTargetsVisible()`'s other callers (compass ticks, filter changes) — bails immediately whenever `state.searchScreenOpen && state.searchHighlightResults.length`, leaving the camera exactly where the search fit last put it. Without this, the ring reappeared on the next fix or tick regardless of how the search fit had just settled — the map jumping back to Nearby a moment or two after correctly framing a search, without the query ever actually losing its matches. Once a query genuinely returns nothing, `state.searchHighlightResults.length` is zero and this guard no longer applies, so a GPS fix or tick is free to frame the ring again exactly as the "no matches" fallback above does explicitly.
